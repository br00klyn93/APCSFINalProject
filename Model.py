...
class Model:
	...
	def setupCTC(self):
		...
		if self.decoderType == DecoderType.BestPath:
			...
			word_beam_search_module = tf.load_op_library('/app/static/TFWordBeamSearch.so')

			# prepare information about language (dictionary, characters in dataset, characters forming words)
			chars = str().join(self.charList)
			wordChars = open('static/model/wordCharList.txt').read().splitlines()[0]
			corpus = open('static/data/corpus.txt').read()
	...
	def save(self):
		"save model to file"
		self.snapID += 1
		self.saver.save(self.sess, 'static/model/snapshot', global_step=self.snapID)
